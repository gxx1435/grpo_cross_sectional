> 本文档记录**不选 Top30、全股池 long-only simplex 配权**实验（预测 `lstm` minute-only，生成 MLP / Gaussian / FM / Diffusion / SS-FM，强化学习 PPO/GRPO）。
> 与同月 Top30 选股实验的 `实验分析.md` 分开；缺测一律 `-`，不补 0、不编造。

# 2026-05 全股池配权 Sequential OOS 实验分析

- 状态: **进行中（12/15 单元已写入）**
- OOS: `sequential_oos_weekly_retrain`
- 配权=全股池 long-only simplex（不做 Top30 截断），cost_bps=3.0，primary=`gated_residual`
- 缺测一律 `-`，不补 0、不编造。

## 固定颜色

| 模型 | 颜色 |
| --- | --- |
| lstm | #4C78A8 |

预测骨干日收益曲线用 `pred_<模型>_softmax_all`，颜色与上表相同。

## 实验进度

| unit | model | status |
| --- | --- | --- |
| A 预测 | lstm | - |
| B 组合/all | equal_weight | 已写入 |
| B 组合/all | mvo | 已写入 |
| B 组合/all | max_sharpe | 已写入 |
| B 组合/all | risk_parity | 已写入 |
| B 组合/all | kelly | 已写入 |
| C 生成/all | mlp | 已写入 |
| C 生成/all | gaussian | 已写入 |
| C 生成/all | standard_fm | 已写入 |
| C 生成/all | diffusion | 已写入 |
| C 生成/all | ssfm | 已写入 |
| D RL/all | ssfm_ppo | 已写入 |
| D RL/all | ssfm_grpo | 已写入 |
| E G/all | G=8 | - |
| E G/all | G=16 | - |

## 实验设定

- Walk-forward：Train=T-13..T-2，Val=T-1，Test=当月。
- Sequential OOS：周五收盘重训 primary + MLP / Gaussian / FM / Diffusion / SS-FM + PPO/GRPO，下周一生效，不回写本周决策。
- 配权在当日有效中证500全股池上完成，不再做 Top30 截断。
- 实验 E：SS-FM 候选数 G∈{8,16}。按周五生效窗口加载当时的 primary / SS-FM，当日采样后按 pred_utility 选 1 条。
- 标签 y = Close_{t+1}/Close_t - 1（下一日 close-to-close）。高严重泄漏 FAIL 的月份不进 final_summary。

## Validation（回归 / 排序）

### 各模型 BEST（按该模型 Val MSE 最低的 epoch）

| model | MSE | MAE | IC | RankIC | topk_f1 | topk_precision | topk_recall | dir_acc | dir_f1 | dir_precision | dir_recall | top30_hit_rate | n |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| lstm | - | - | - | - | - | - | - | - | - | - | - | - | - |

列含义: `MSE`/`MAE` 是 ŷ 对 y 的误差；`IC` 是 Pearson；`RankIC` 是 Spearman；`topk_f1` 是预测 Top30 ∩ 真实收益 Top30 / 30；`dir_acc` 是涨跌符号一致率；`top30_hit_rate` 是入选 30 只里 y>0 的比例。

### 各 epoch

| model | epoch | MSE | MAE | IC | RankIC | topk_f1 | topk_precision | topk_recall | dir_acc | dir_f1 | dir_precision | dir_recall | top30_hit_rate | n |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| lstm | 1 | - | - | - | - | - | - | - | - | - | - | - | - | - |
| lstm | 2 | - | - | - | - | - | - | - | - | - | - | - | - | - |
| lstm | 3 | - | - | - | - | - | - | - | - | - | - | - | - | - |

## Test 预测指标（日均）

| model | MSE | MAE | IC | RankIC | topk_f1 | topk_precision | topk_recall | dir_acc | dir_f1 | dir_precision | dir_recall | top30_hit_rate | top30_excess | n |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| lstm | - | - | - | - | - | - | - | - | - | - | - | - | - | - |

## 组合绩效

| model | total_net_return | ann_return | sharpe | max_drawdown | mean_turnover | mean_cost | n_days |
| --- | --- | --- | --- | --- | --- | --- | --- |
| pred_lstm_softmax_all | - | - | - | - | - | - | - |
| universe_ew_all | - | - | - | - | - | - | - |
| baseline_equal_weight_all | - | - | - | - | - | - | - |
| baseline_mvo_all | - | - | - | - | - | - | - |
| baseline_max_sharpe_all | - | - | - | - | - | - | - |
| baseline_risk_parity_all | - | - | - | - | - | - | - |
| baseline_kelly_all | - | - | - | - | - | - | - |
| baseline_equal_weight | 0.0124 | - | 105.5485 | 0.0000 | 0.1667 | 0.0000 | 3 |
| baseline_mvo | 0.0143 | - | 7.5261 | 0.0059 | 0.5000 | 0.0001 | 3 |
| baseline_max_sharpe | 0.0091 | - | 10.4295 | 0.0020 | 0.7358 | 0.0002 | 3 |
| baseline_risk_parity | 0.0125 | - | 116.6783 | 0.0000 | 0.1783 | 0.0001 | 3 |
| baseline_kelly | 0.0143 | - | 7.5261 | 0.0059 | 0.5000 | 0.0001 | 3 |
| baseline_score_softmax_all | - | - | - | - | - | - | - |
| gen_mlp_all | - | - | - | - | - | - | - |
| gen_gaussian_all | - | - | - | - | - | - | - |
| gen_standard_fm_all | - | - | - | - | - | - | - |
| gen_diffusion_all | - | - | - | - | - | - | - |
| gen_ssfm_all | - | - | - | - | - | - | - |
| gen_mlp | 0.0169 | - | 11.1787 | 0.0012 | 0.4847 | 0.0001 | 3 |
| gen_gaussian | 0.0181 | - | 7.6585 | 0.0067 | 0.5456 | 0.0002 | 3 |
| gen_standard_fm | -0.0148 | - | -9.0015 | 0.0195 | 0.4888 | 0.0001 | 3 |
| gen_diffusion | -0.0093 | - | -4.5417 | 0.0177 | 0.7266 | 0.0002 | 3 |
| gen_ssfm | 0.0164 | - | 10.0172 | 0.0053 | 0.7706 | 0.0002 | 3 |
| rl_ssfm_ppo_all | - | - | - | - | - | - | - |
| rl_ssfm_grpo_all | - | - | - | - | - | - | - |
| rl_ssfm_ppo | 0.0087 | - | 8.8712 | 0.0043 | 0.3464 | 0.0001 | 3 |
| rl_ssfm_grpo | 0.0017 | - | 3.1785 | 0.0035 | 0.3547 | 0.0001 | 3 |
| ssfm_G8_all | - | - | - | - | - | - | - |
| ssfm_G16_all | - | - | - | - | - | - | - |
| ssfm_G8 | 0.0158 | - | 7.3991 | 0.0088 | 0.7455 | 0.0002 | 3 |
| ssfm_G16 | 0.0119 | - | 7.8601 | 0.0052 | 0.7146 | 0.0002 | 3 |

## 日均换手

| model | mean_turnover |
| --- | --- |
| pred_lstm_softmax_all | - |
| universe_ew_all | - |
| baseline_equal_weight_all | - |
| baseline_mvo_all | - |
| baseline_max_sharpe_all | - |
| baseline_risk_parity_all | - |
| baseline_kelly_all | - |
| baseline_equal_weight | 0.1667 |
| baseline_mvo | 0.5000 |
| baseline_max_sharpe | 0.7358 |
| baseline_risk_parity | 0.1783 |
| baseline_kelly | 0.5000 |
| baseline_score_softmax_all | - |
| gen_mlp_all | - |
| gen_gaussian_all | - |
| gen_standard_fm_all | - |
| gen_diffusion_all | - |
| gen_ssfm_all | - |
| gen_mlp | 0.4847 |
| gen_gaussian | 0.5456 |
| gen_standard_fm | 0.4888 |
| gen_diffusion | 0.7266 |
| gen_ssfm | 0.7706 |
| rl_ssfm_ppo_all | - |
| rl_ssfm_grpo_all | - |
| rl_ssfm_ppo | 0.3464 |
| rl_ssfm_grpo | 0.3547 |
| ssfm_G8_all | - |
| ssfm_G16_all | - |
| ssfm_G8 | 0.7455 |
| ssfm_G16 | 0.7146 |

## 图（颜色已固定，无数据时只画轴）

### 预测骨干累计收益

![预测骨干累计收益](figures/nav_pred.png)

固定颜色: `pred_lstm_ew` `#4C78A8`。

### 组合基线累计收益

![组合基线累计收益](figures/nav_port.png)

固定颜色: `baseline_equal_weight` `#4C78A8`；`baseline_mvo` `#F58518`；`baseline_max_sharpe` `#E45756`；`baseline_risk_parity` `#72B7B2`；`baseline_kelly` `#B279A2`。

### 生成模型累计收益

![生成模型累计收益](figures/nav_gen.png)

固定颜色: `gen_mlp` `#9D755D`；`gen_gaussian` `#BAB0AC`；`gen_standard_fm` `#17BECF`；`gen_diffusion` `#F2CF5B`；`gen_ssfm` `#B279A2`。

### RL 累计收益

![RL 累计收益](figures/nav_rl.png)

固定颜色: `gen_ssfm` `#B279A2`；`rl_ssfm_ppo` `#E45756`；`rl_ssfm_grpo` `#4C78A8`。

### G 曲线累计收益

![G 曲线累计收益](figures/nav_g_candidates.png)

固定颜色: `ssfm_G8` `#9D755D`；`ssfm_G16` `#BAB0AC`。

### 日均换手

![日均换手](figures/mean_turnover.png)

固定颜色: `pred_lstm_softmax_all` `#4C78A8`；`universe_ew_all` `#7F7F7F`；`baseline_equal_weight_all` `#4C78A8`；`baseline_mvo_all` `#F58518`；`baseline_max_sharpe_all` `#E45756`；`baseline_risk_parity_all` `#72B7B2`；`baseline_kelly_all` `#B279A2`；`baseline_equal_weight` `#4C78A8`；`baseline_mvo` `#F58518`；`baseline_max_sharpe` `#E45756`；`baseline_risk_parity` `#72B7B2`；`baseline_kelly` `#B279A2`；`baseline_score_softmax_all` `#7F7F7F`；`gen_mlp_all` `#9D755D`；`gen_gaussian_all` `#BAB0AC`；`gen_standard_fm_all` `#17BECF`；`gen_diffusion_all` `#F2CF5B`；`gen_ssfm_all` `#B279A2`；`gen_mlp` `#9D755D`；`gen_gaussian` `#BAB0AC`；`gen_standard_fm` `#17BECF`；`gen_diffusion` `#F2CF5B`；`gen_ssfm` `#B279A2`；`rl_ssfm_ppo_all` `#E45756`；`rl_ssfm_grpo_all` `#4C78A8`；`rl_ssfm_ppo` `#E45756`；`rl_ssfm_grpo` `#4C78A8`；`ssfm_G8_all` `#9D755D`；`ssfm_G16_all` `#BAB0AC`；`ssfm_G8` `#9D755D`；`ssfm_G16` `#BAB0AC`。

### Val IC

![Val IC](figures/val_ic.png)

固定颜色: `lstm` `#4C78A8`。

### Val RankIC

![Val RankIC](figures/val_rankic.png)

固定颜色: `lstm` `#4C78A8`。

### Val F1@Top30

![Val F1@Top30](figures/val_f1.png)

固定颜色: `lstm` `#4C78A8`。

### Val MSE

![Val MSE](figures/val_mse.png)

固定颜色: `lstm` `#4C78A8`。

### Val IC 各 epoch

![Val IC 各 epoch](figures/val_ic_epochs.png)

固定颜色: `lstm` `#4C78A8`。

### Val RankIC 各 epoch

![Val RankIC 各 epoch](figures/val_rankic_epochs.png)

固定颜色: `lstm` `#4C78A8`。

### Val F1@Top30 各 epoch

![Val F1@Top30 各 epoch](figures/val_f1_epochs.png)

固定颜色: `lstm` `#4C78A8`。

### Val MSE 各 epoch

![Val MSE 各 epoch](figures/val_mse_epochs.png)

固定颜色: `lstm` `#4C78A8`。

### Test IC

![Test IC](figures/ic_ts.png)

固定颜色: `lstm` `#4C78A8`。

### Test RankIC

![Test RankIC](figures/rankic_ts.png)

固定颜色: `lstm` `#4C78A8`。

### Test F1@Top30

![Test F1@Top30](figures/f1_ts.png)

固定颜色: `lstm` `#4C78A8`。

### Test Hit@Top30

![Test Hit@Top30](figures/hit_ts.png)

固定颜色: `lstm` `#4C78A8`。

## 图对应数据表

- 累计收益 ← `tables/backtest_daily.csv` 的 `net_return` 累加。
- 绩效 / 换手 ← `tables/performance_summary.csv`。
- Val ← `tables/val_best.csv`、`tables/val_epochs.csv`。
- Test 预测 ← `tables/prediction_metrics_mean.csv`。

## 分析

以下只讨论**已经写入数字**的行；单元格为 `-` 的表示该实验尚未跑完，不编造。
来源: month 2026-05。OOS=`strict_fixed_oos`，费用=3.0bps，配权池=全股池（无 Top30）。

已有数据的对象: `equal_weight`, `mvo`, `max_sharpe`, `risk_parity`, `kelly`, `mlp`, `gaussian`, `standard_fm`, `diffusion`, `ssfm`, `ssfm_ppo`, `ssfm_grpo`。
- Test 日均 IC 目前最高: `gated_residual` = -0.0461。
- 已回测净值目前最高: `ssfm_G32` 累计净收益 0.0468，Sharpe 45.3693。
- 实验 E（G 候选数，top30）累计净收益最高: `ssfm_G32` = 0.0468（G=8 0.0158；G=16 0.0119）。

## 实验 E：G 候选数

全股池。每个交易日加载当时周五生效的 primary / SS-FM，采 G 条权重后按 pred_utility 选 1 条。与默认 `gen_ssfm`（固定 default_g）不是同一条抽样路径。

| G | 累计净收益 | Sharpe | 最大回撤 | 日均换手 | 天数 |
| --- | --- | --- | --- | --- | --- |
| G=8 | 0.0158 | 7.3991 | 0.0088 | 0.7455 | 3 |
| G=16 | 0.0119 | 7.8601 | 0.0052 | 0.7146 | 3 |

本月网格里累计净收益最好的是 `ssfm_G32`。更大的 G 没有单调变好。
