"""Train PPO / GRPO from a pretrained SS-FM checkpoint path for one month."""
...
def main() -> None:
    ...
    out = resolve_path(cfg, cfg["paths"]["results_dir"]) / "entry_train_rl"
    tee = Tee(out / "run.log")
    ...
    run_month(
        ...
        models_filter=[cfg["prediction"]["primary_model"]], skip_rl=False, skip_gen=False,
    )
    log(str(gpu.stop()))
