#!/usr/bin/env python3
"""SP500 Top30 FM Meta stability: 20 seeds × G-grid × 4 aggregations.
...
def write_report(dest_root: Path, year_csv: Path, out_tag: str, noise_seeds: List[int]) -> None:
    ...
    title = f"Top30 FM Meta stability：G×聚合（{len(noise_seeds)} seeds）"
    plot_grouped_bars(
    ...
    L = []
    a = L.append
    a(f"## Top30 FM Meta stability：G×聚合（{len(noise_seeds)} seeds）")
    a("")
    a(
        "设定：冻结 Top30 最新 Alpha+standard FM（**不训练**）；配权 **Top30**；"
    ...
    if len(summary):
        ...
        analysis = "\n".join(
            [
                f"### Top30 FM Meta stability（{len(noise_seeds)} seeds）结论",
        ...
        analysis = f"### Top30 FM Meta stability（{len(noise_seeds)} seeds）结论\n- 无有效结果。\n"
    ...
    marker = f"## Top30 FM Meta stability：G×聚合（{len(noise_seeds)} seeds）\n"
    amark = f"### Top30 FM Meta stability（{len(noise_seeds)} seeds）结论\n"
    ...
    (dest_root / "analysis" / "fm_meta_stability_seeds报告片段.md").write_text(
...
def main():
    ...
    ap.add_argument("--out-tag", default="strict_fixed_oos_top30_fm_stability_meta_seeds")
    ...
    with open(log_path, "a", encoding="utf-8") as flog:
        ...
        for month in months:
            _log(f"==== TOP30 FM-Meta-stability-seeds month={month} ====")
            t0 = time.time()
        ...
        _log(
            f"REQUESTED MONTHS SP500-TOP30-FM-META-STABILITY-SEEDS DONE "
