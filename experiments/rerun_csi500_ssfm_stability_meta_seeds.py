#!/usr/bin/env python3
"""CSI500 Top30 SS-FM Meta stability: 20 seeds × G-grid × 4 aggregations.
...
def run_month(
    ...
    log(f"==== {month} test cache pool={POOL} K={k} SS-FM Meta n_seeds={len(noise_seeds)} (freeze) ====")
...
def write_report(dest_root: Path, year_csv: Path, out_tag: str, noise_seeds: List[int], g_list: List[int]) -> None:
    ...
    title = f"CSI500 SS-FM Meta stability：G×聚合（{len(noise_seeds)} seeds）"
    ...
    L = []
    a = L.append
    a(f"# CSI500 SS-FM Meta stability：G×聚合（{len(noise_seeds)} seeds）")
    a("")
    a(
        "设定：冻结 CSI500 Top30 最新 Alpha+SS-FM（**不训练**）；"
    ...
    text = "\n".join(L)
    (dest_root / "analysis" / "CSI500_SSFM_Meta_stability报告.md").write_text(text, encoding="utf-8")
    Path("results/CSI500/CSI500_SSFM_Meta_stability实验报告.md").write_text(
        text.replace("(figures/", f"({out_tag}/analysis/figures/"), encoding="utf-8"
    )
    log("wrote SS-FM Meta stability report")
...
def main():
    ...
    ap.add_argument("--out-tag", default="strict_fixed_oos_ssfm_stability_meta_seeds")
    ...
    with open(log_path, "a", encoding="utf-8") as flog:
        ...
        _log(
            ...
            f"modes={[m for m,_ in MODES]} Meta SS-FM freeze out={dest_root}"
        )
        for month in months:
            ...
            if month_csv.is_file():
                ...
                continue
            _log(f"==== CSI SS-FM-Meta-stability month={month} ====")
            t0 = time.time()
        ...
        write_report(dest_root, year_csv, args.out_tag, noise_seeds, G_LIST)
        _log(
            f"REQUESTED MONTHS CSI500-SSFM-META-STABILITY-SEEDS DONE "
