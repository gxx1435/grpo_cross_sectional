...
def run_month(cfg, store, device, month: str, src_root: Path, dest_root: Path, g: int) -> Path:
    ...
    dest_exp = dest_month / f"{src_exp.name}_rl_r06"
    ...
    for it in test_cache:
        ...
        ws.append(apply_valid_mask(wsel, it["valid"][it["idx"]]))
    df_ppo = backtest_items(test_cache, ws, cfg, f"rl_ssfm_ppo_cand_G{g}_r06_{POOL}", OOS, g)
    parts.append(df_ppo)
...
def write_report(dest_root: Path, year_csv: Path, g: int, out_tag: str) -> None:
    ...
    rename = {
        ...
        f"rl_ssfm_ppo_cand_G{g}_r06_{POOL}": f"SS-FM+PPO (cand) G={g} r06",
    ...
    pref = "rl_r06_vs_ssfm"
    title = f"Top30 G={g} reward-r06：SS-FM vs PPO/GRPO"
    ...
    nav.reset_index().to_csv(tab / "rl_r06_vs_ssfm.csv", index=False)

    n_days = int(nav["n_days"].max())
    L = []
    a = L.append
    a(f"## Top30 G={g} RL reward-r06：SS-FM vs PPO / GRPO")
    a("")
    a(
    ...
    marker = f"## Top30 G={g} RL reward-r06：SS-FM vs PPO / GRPO\n"
    amark = f"### Top30 G={g} RL reward-r06 结论\n"
    import re
    ...
    (dest_root / "analysis" / "rl_r06报告片段.md").write_text(
...
def main():
    ...
    ap.add_argument("--out-tag", default="strict_fixed_oos_top30_rl_r06")
    ...
    with open(log_path, "a", encoding="utf-8") as flog:
        ...
        for month in months:
            _log(f"==== TOP30 RL-r06 month={month} ====")
            t0 = time.time()
        ...
        _log(f"REQUESTED MONTHS SP500-TOP30-RL-R06 DONE hours={(time.time()-t_all)/3600:.2f} out={dest_root}")
