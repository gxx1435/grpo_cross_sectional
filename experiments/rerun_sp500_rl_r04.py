#!/usr/bin/env python3
"""SP500 Top30 RL reward ablation: SS-FM vs cand-PPO vs GRPO-init.

Reward (composite, robust-z):
  r = 0.4 z(net) + 0.4 z(causal Sharpe) - 0.05 z(turnover) - 0.05 z(smooth MDD)

Pure SS-FM: G=32 Meta (samples ∪ teachers → pred_utility).
PPO: candidate-as-feature; GRPO: SS-FM distill init; bc_coef=0.
"""
...
def _apply_reward(cfg: dict) -> dict:
    ...
    # keep zmax/eps from base if present
    return out
...
def run_month(cfg, store, device, month: str, src_root: Path, dest_root: Path, g: int) -> Path:
    ...
    dest_exp = dest_month / f"{src_exp.name}_rl_r04"
    ...
    log(f"==== {month} PPO cand reward={REWARD} ====")
    t0 = time.time()
    ...
    for it in test_cache:
        cond = torch.from_numpy(it["cond"]).to(device)
        with torch.no_grad():
            cands = sample_ss_fm_mixed(ssfm, cond, g, n_steps)
        state = pol_ppo.encode(cond, cands)
        w, _, _ = sample_portfolios_from_state(pol_ppo, state, g, float(cfg_rl["rl"]["noise_std"]))
        arr = w.detach().cpu().numpy()
        _, wsel = select_candidate(arr, _alpha_pool(it), it["sigma"], cfg)
        ws.append(apply_valid_mask(wsel, it["valid"][it["idx"]]))
    df_ppo = backtest_items(test_cache, ws, cfg, f"rl_ssfm_ppo_cand_G{g}_r04_{POOL}", OOS, g)
    parts.append(df_ppo)
    ...
    for it in test_cache:
        ...
        _, wsel = select_candidate(arr, _alpha_pool(it), it["sigma"], cfg)
        ws.append(apply_valid_mask(wsel, it["valid"][it["idx"]]))
    df_grpo = backtest_items(test_cache, ws, cfg, f"rl_ssfm_grpo_init_G{g}_r04_{POOL}", OOS, g)
...
def write_report(dest_root: Path, year_csv: Path, g: int, out_tag: str) -> None:
    ...
    rename = {
        ...
        f"rl_ssfm_ppo_cand_G{g}_r04_{POOL}": f"SS-FM+PPO (cand) G={g} r04",
        f"rl_ssfm_grpo_init_G{g}_r04_{POOL}": f"SS-FM+GRPO (SSFM init) G={g} r04",
    }
    ...
    names = [
        f"SS-FM G={g} Meta",
        f"SS-FM+PPO (cand) G={g} r04",
        f"SS-FM+GRPO (SSFM init) G={g} r04",
    ]
    cm = {
    ...
    pref = "rl_r04_vs_ssfm"
    title = f"Top30 G={g} reward-r04：SS-FM vs PPO/GRPO"
    ...
    nav.reset_index().to_csv(tab / "rl_r04_vs_ssfm.csv", index=False)

    n_days = int(nav["n_days"].max())
    L = []
    a = L.append
    a(f"## Top30 G={g} RL reward-r04：SS-FM vs PPO / GRPO")
    a("")
    a(
        f"设定：冻结 Top30 Alpha+SS-FM；配权 **Top30**；**G={g}**。"
        "Pure SS-FM 为 Meta 选仓（样本 ∪ Teacher 闭式解 → `pred_utility`）。"
        "PPO=cand 特征；GRPO=SSFM init；`bc_coef=0`。"
    )
    a("")
    a(
    ...
    analysis = "\n".join(
        [
            f"### Top30 G={g} RL reward-r04 结论",
            f"- 单利：SS-FM Meta `{pure:.4f}`；cand-PPO `{ppo:.4f}`（{'胜' if ppo > pure else '负'}）；"
    ...
    # report paths with different relative figure roots
    targets = [
    ...
    marker = f"## Top30 G={g} RL reward-r04：SS-FM vs PPO / GRPO\n"
    amark = f"### Top30 G={g} RL reward-r04 结论\n"
    import re
    ...
    (dest_root / "analysis" / "rl_r04报告片段.md").write_text(
...
def main():
    ...
    ap.add_argument("--out-tag", default="strict_fixed_oos_top30_rl_r04")
    ...
    with open(log_path, "a", encoding="utf-8") as flog:
        ...
        _log(f"CONFIG pool={POOL} g={g} reward={REWARD} out={dest_root}")
        for month in months:
            _log(f"==== TOP30 RL-r04 month={month} ====")
            t0 = time.time()
        ...
        _log(f"REQUESTED MONTHS SP500-TOP30-RL-R04 DONE hours={(time.time()-t_all)/3600:.2f} out={dest_root}")
