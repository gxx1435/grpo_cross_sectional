...
from portfolio.baselines import all_baselines, score_softmax
...
def build_cond(alpha_k: np.ndarray, mu: np.ndarray, sigma: np.ndarray, compact: bool = False) -> np.ndarray:
    ...
    def z(x):
        ...
        return ((x - float(np.mean(x))) / (float(np.std(x)) + 1e-6)).astype(np.float32)

    parts = [z(alpha_k), z(mu), z(vol)]
    if compact:
        return np.concatenate(parts).astype(np.float32)
    iu = np.triu_indices(sigma.shape[0])
    vech = sigma[iu].astype(np.float32)
    vech = vech / (float(np.std(vech)) + 1e-6)
    return np.concatenate(parts + [vech]).astype(np.float32)


def cond_dim(k: int, compact: bool = False) -> int:
    if compact:
        return 3 * int(k)
    return int(k) + int(k) + int(k) + int(k) * (int(k) + 1) // 2
...
def make_alloc_item(store, day, pred, valid, y, ameta, cfg, pool: str) -> dict:
    ...
    daily = store.daily_returns_to(day)
    mu, sigma = hist_mu_sigma(daily, day, names, int(cfg["portfolio"]["hist_risk_lookback_days"]))
    teach = teachers(mu, sigma, float(cfg["portfolio"]["max_weight"]), float(cfg["ssfm"]["risk_aversion"]))
    bases = all_baselines(mu, sigma, pred[idx], float(cfg["portfolio"]["max_weight"]))
    valid_k = valid[idx]
    teach = {n: apply_valid_mask(w, valid_k) for n, w in teach.items()}
    bases = {n: apply_valid_mask(w, valid_k) for n, w in bases.items()}
    bases["score_softmax"] = apply_valid_mask(score_softmax(pred[idx], valid_k, tau), valid_k)
    return {
        "asof": day,
        "idx": idx,
        "names": names,
...
def build_state_cache(store, model, days, cfg, device, pool: str = "top30") -> List[dict]:
    items = []
    bar = Progress(max(len(days), 1), f"STATE 构造{pool}/Teacher", log_every=15)
    for i, day in enumerate(days):
        try:
            y, _ = store.labels(day)
...
def try_load_alloc_ckpts(init_dir: Path, dest_dir: Path, cfg: dict, device, pool: str, k: int, compact: bool) -> Optional[Dict[str, Any]]:
    ...
    paths = [init_dir / f"gen_{n}_{pool}.pt" for n in gen_names] + [init_dir / f"rl_{a}_{pool}.pt" for a in algos]
    if not all(p.is_file() for p in paths):
        return None
    gen = _new_gen_models(cfg, device, k, compact)
    ...
    for algo in algos:
        ...
        rl[algo] = pol
    return {"cache": [], "gen": gen, "rl": rl, "k": k, "compact": compact}


def train_generative(cache, cfg, device, k: Optional[int] = None, compact: bool = False) -> Dict[str, nn.Module]:
...
def distill_and_rl(ss_fm, cache, cfg, device, algo: str, k: Optional[int] = None, compact: bool = False) -> WeightPolicy:
    ...
    for di in range(int(cfg["rl"]["distill_steps"])):
        ...
        with torch.no_grad():
            w = sample_ss_fm_mixed(ss_fm, cond, 8, int(cfg["ssfm"]["n_sample_steps"])).mean(0)
    ...
    for item in cache:
        ...
        hist["sharpe"].append(causal_sharpe(nets[:-1], cfg["portfolio"]["sharpe_min_obs"], 0.0))
        ...
        prev = w
    scalers = fit_reward_scalers({k: np.asarray(v) for k, v in hist.items()}, cfg, fit_range)
    ...
    for ep in range(int(cfg["rl"]["epochs"])):
        ...
        for item in cache:
            ...
            rews = []
            shp = causal_sharpe(hist_net, cfg["portfolio"]["sharpe_min_obs"], 0.0)
            ...
            w_bar = ws.mean(0)
            w_bar = np.clip(w_bar, 0, None)
            w_bar = w_bar / max(float(w_bar.sum()), 1e-12)
            met0 = execute_day(w_bar, item["R"], prev, cfg)
            hist_net.append(met0["net_return"])
            prev = w_bar
            n_upd += 1
            rl_bar.update(ep * len(cache) + n_upd, f"ep{ep+1} {item['asof'].date()}")
        log(f"      {algo.upper()} epoch {ep+1} updates={n_upd} wall={time.time()-t0:.1f}s")
    rl_bar.close()
...
def run_month(
    ...
    out = out_root / oos_mode / f"test_month={month}" / exp_id
    ...
    def fit_one(name: str, train_days, val_days, load_only: bool = False, load_from: Optional[Path] = None) -> nn.Module:
        ...
        stats = train_predictor(
            ...
            device,
            val_days,
            resume_path=ckpt_dir / f"sft_resume_{name}.pt",
        )
        torch.save(model.state_dict(), ckpt_dir / f"alpha_{name}.pt")
    ...
    for name in pred_names:
        ...
        device_models[name] = fit_one(name, w0["train_days"], w0["val_days"])
    ...
    def fit_allocators(days, tag: str) -> None:
        ...
        for pool in pools:
            ...
            if tag == "init" and init_dir is not None:
                ...
                if loaded is not None:
                    ...
                    continue
            cache = build_state_cache(store, primary_model, days, cfg, device, pool=pool)
            if not cache:
    ...
    fit_allocators(w0["train_days"], "init")
    ...
    def maybe_weekly_update(day: pd.Timestamp) -> None:
        ...
        if win is None or win.get("effective_from") is None:
            return
        log(f"    Friday close retrain cutoff={win['cutoff']} effective_from={win['effective_from']} (primary+SS-FM+RL, pools={pools})")
        ...
        try:
            if friday_sft_dir is not None:
                ...
                device_models[primary] = fit_one(primary, win["train_days"], win["val_days"], load_only=True, load_from=friday_sft_dir)
            else:
                device_models[primary] = fit_one(primary, win["train_days"], win["val_days"])
            fit_allocators(win["train_days"], "friday")
        finally:
            for n in parked:
    ...
    for ti, day in enumerate(split["test_days"]):
        ...
        for name in pred_names:
            ...
            idx = select_topk(pred, valid, int(cfg["portfolio"]["top_k"]))
            ...
            item["y_c2c"] = y_c2c
            item["model"] = name
            test_items_by_model[name].append(item)
            if name == primary:
                item_primary = item
                if "top30" in test_cache_by_pool:
                    test_cache_by_pool["top30"].append(item)
                if "all" in test_cache_by_pool:
                    test_cache_by_pool["all"].append(make_alloc_item(store, day, pred, valid, y, pmeta, cfg, "all"))
    ...
    for name, items in test_items_by_model.items():
        ...
        daily_all.append(backtest_items(items, ws, cfg, f"pred_{name}_ew_top30", oos_mode, 1))
        _flush_partial_daily(f"A-backtest-{name}-top30")
        if "all" in pools:
            items_all = []
            ws_all = []
            for it in items:
                w = apply_valid_mask(score_softmax(it["alpha"], it["valid"], tau), it["valid"])
                row = dict(it)
                row["R"] = it["y"]
                row["names"] = list(store.kept)
