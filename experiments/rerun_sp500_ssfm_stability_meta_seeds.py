#!/usr/bin/env python3
"""SP500 Top30 SS-FM Meta stability across 20 noise seeds × G-grid × aggregations.

Freeze latest Alpha+SS-FM (no training).
Meta pool = G SS-FM samples ∪ Teacher (MVO/MaxSharpe/RP).
...
def run_month(
    ...
    dest_exp = dest_month / f"{src_exp.name}_ssfm_stability_meta_seeds"
    ...
    log(f"==== {month} test cache pool={POOL} K={k} Meta n_seeds={len(noise_seeds)} (freeze) ====")
    ...
    for it in test_cache:
        ...
        for g in G_LIST:
            for seed in noise_seeds:
                ...
                with torch.no_grad():
                    ws = sample_ss_fm_mixed_seeded(ssfm, cond, g, n_steps, day_seed).detach().cpu().numpy()
...
def write_report(dest_root: Path, year_csv: Path, out_tag: str, noise_seeds: List[int]) -> None:
    ...
    seed_df.to_csv(tab / "ssfm_meta_stability_seeds_raw.csv", index=False)
    ...
    summary.to_csv(tab / "ssfm_meta_stability_seeds_summary.csv", index=False)
    ...
    pivot_mu.to_csv(tab / "ssfm_meta_stability_seeds_pivot_return_mean.csv")
    pivot_sd.to_csv(tab / "ssfm_meta_stability_seeds_pivot_return_std.csv")

    mode_colors = {
        "Pred-Utility": color_for("gen_ssfm"),
        "Mean": "#E45756",
        "Top-k": "#4C78A8",
        "Softmax": "#F58518",
    }
    ...
    pref = "ssfm_meta_stability_seeds"
    title = f"Top30 SS-FM Meta stability：G×聚合（{len(noise_seeds)} seeds）"
    plot_grouped_bars(
    ...
    for seed in noise_seeds[: min(8, len(noise_seeds))]:  # plot up to 8 seeds to avoid clutter
        ...
        SERIES_COLORS[name] = color_for("gen_ssfm")
    if sn:
        plot_series(
    ...
    L = []
    a = L.append
    a(f"## Top30 SS-FM Meta stability：G×聚合（{len(noise_seeds)} seeds）")
    a("")
    a(
        "设定：冻结 Top30 最新 Alpha+SS-FM（**不训练**）；配权 **Top30**；"
        "**Meta 候选 = G 条 SS-FM ∪ Teacher（MVO/MaxSharpe/RP）**；"
    ...
    if len(summary):
        ...
        analysis = "\n".join(
            [
                f"### Top30 SS-FM Meta stability（{len(noise_seeds)} seeds）结论",
        ...
        analysis = f"### Top30 SS-FM Meta stability（{len(noise_seeds)} seeds）结论\n- 无有效结果。\n"
    ...
    marker = f"## Top30 SS-FM Meta stability：G×聚合（{len(noise_seeds)} seeds）\n"
    amark = f"### Top30 SS-FM Meta stability（{len(noise_seeds)} seeds）结论\n"
    ...
    (dest_root / "analysis" / "ssfm_meta_stability_seeds报告片段.md").write_text(
...
def main():
    ...
    ap.add_argument("--out-tag", default="strict_fixed_oos_top30_ssfm_stability_meta_seeds")
    ...
    with open(log_path, "a", encoding="utf-8") as flog:
        ...
        for month in months:
            _log(f"==== TOP30 SS-FM-Meta-stability-seeds month={month} ====")
            t0 = time.time()
        ...
        _log(
            f"REQUESTED MONTHS SP500-TOP30-SSFM-META-STABILITY-SEEDS DONE "
